...
const investmentSchema = require('./investmentSchema'); // Ensure this schema is defined and exported correctly
...
const fundSchema = new mongoose.Schema({
  ...
  app.get("/fund", async (req, res) => {
    try {
        // Get the investment type selected by the user from the query parameters
        const investmentType = req.query.investmentType;
console.log(investmentType);
        // Filter funds based on the selected investment type
        const filteredFunds = await Fund.find({ Category: investmentType });
...
app.get("/", (req, res) => {
    res.sendFile(path.join(__dirname, "Plandetails.html"));
});

// Serve plantype.html
app.get("/plantype", (req, res) => {
    res.sendFile(path.join(__dirname, "plantype.html"));
});

// Serve goaloptions.html
app.get("/goaloptions", (req, res) => {
    res.sendFile(path.join(__dirname, "goaloptions.html"));
});
